...
int main(int argc, char* argv[]){
    ...
    if(argc < 4){
        ...
        sscanf(argv[1], "%llx", &baseaddr);
        sscanf(argv[2], "%s", &RegNumArgc );
        sscanf(argv[3], "%d", &RegNum );
        if(RegNumArgc[0] != '-' || (RegNumArgc[1] != 'n' && RegNumArgc[1] != 'N') ){
            printf("usage*(1): rdaxi reg_addr(address in hex) -n number_of_read_registers\n\n");
            return 0;
        }
    }
    ...
    for(i = 0; i < RegNum; i ++){
        ...
        printf("[%d] AXI reg 0x%08llx = 0x%08llx (hex) = %lu (dec)\n", i, addr, v, v);
